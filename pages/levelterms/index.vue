  ...
  <div>
    ...
    <div v-else>
      <pageheader
        ...
        urltxt="Add level_term"
      ></pageheader>

      <div class="col-12"></div>
      <v-server-table
        :url="serverurl + 'admin/levelterms?type=' + selectData"
        :columns="columns"
        :options="options"
      >
        <div
        ...
        >
          <a
            v-show="$can('level_term_update')"
            href=""
            @click.prevent="edit(row)"
            title="level_term Edit"
            ><font-awesome-icon :icon="['far', 'edit']"
          /></a>

          <a
            v-show="$can('level_term_delete')"
            href=""
            title="level_term Delete"
...
export default {
  ...
  data() {
    return {
      ...
      error: false,
      selectData: 'all',
      columns: ['id', 'name', 'department_name', 'slug', 'actions'],
      options: {
        perPage: 10,
  ...
  methods: {
    edit(data) {
      console.log(data)
      this.$nuxt.$router.push('/levelterms/edit/' + data.id)
    },
