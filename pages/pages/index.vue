...
<template>
  <div>
    ...
    <div v-else>
      ...
      <v-server-table
        :url="serverurl + 'admin/pages?type=' + selectData"
        :columns="columns"
        :options="options"
      >
        <div slot="status" slot-scope="{ row }">
          <span v-if="row.status == 1" class="badge bg-primary">Active</span>
          <span v-if="row.status == 0" class="badge bg-info">Draft</span>
        </div>
        <div
          slot="actions"
          slot-scope="{ row }"
...
export default {
  ...
  data() {
    return {
      ...
      error: false,
      selectData: 'all',
      columns: ['id', 'title', 'slug', 'status', 'actions'],
      options: {
        perPage: 10,
  ...
  methods: {
    edit(data) {
      console.log(data)
      this.$nuxt.$router.push('/pages/edit/' + data.id)
    },
    ...
    handleDelete(data, i) {
      ...
      if (confirm(msg)) {
        ...
        this.$axios
          .delete(
            '/admin/pages/' + data.id + '?forcedelete=' + forcedelete.toString()
          )
          .then((res) => {
            vm.loading = false
