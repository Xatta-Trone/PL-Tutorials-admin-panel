...
<template>
  <div>
    ...
    <div v-else>
      ...
      <v-server-table
        :url="serverurl + 'admin/books?type=' + selectData"
        :columns="columns"
        :options="options"
      >
...
export default {
  ...
  data() {
    return {
      ...
      error: false,
      selectData: 'all',
      columns: ['id', 'name', 'status', 'admin_name', 'actions'],
      options: {
        perPage: 10,
  ...
  methods: {
    edit(data) {
      console.log(data)
      this.$nuxt.$router.push('/books/edit/' + data.id)
    },
    ...
    handleDelete(data, i) {
      ...
      if (confirm(msg)) {
        ...
        this.$axios
          .delete(
            '/admin/books/' + data.id + '?forcedelete=' + forcedelete.toString()
          )
          .then((res) => {
            vm.loading = false
