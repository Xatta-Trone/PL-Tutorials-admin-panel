...
<template>
  <div>
    ...
    <div v-else>
      ...
      <v-server-table
        :url="serverurl + 'admin/bandays?type=' + selectData"
        :columns="columns"
        :options="options"
      >
        <div
...
export default {
  ...
  methods: {
    edit(data) {
      console.log(data)
      this.$nuxt.$router.push('/bandays/edit/' + data.id)
    },
    ...
    handleDelete(data, i) {
      ...
      if (confirm(msg)) {
        ...
        this.$axios
          .delete(
            '/admin/bandays/' + data.id + '?forcedelete=' + forcedelete.toString()
          )
          .then((res) => {
            vm.loading = false
