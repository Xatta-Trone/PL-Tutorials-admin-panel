  ...
  <div>
    ...
    <div v-else>
      <pageheader
        ...
        urltxt="Add Softwares"
      ></pageheader>

      <div class="col-12"></div>
      <v-server-table
        :url="serverurl + 'admin/softwares?type=' + selectData"
        :columns="columns"
        :options="options"
      >
        ...
        >
          ...
          <NuxtLink
            v-show="$can('software_update')"
            :to="'/softwares/edit/' + row.id"
            title="software Edit"
            ><font-awesome-icon :icon="['far', 'edit']"
          /></NuxtLink>

          <a
            v-show="$can('software_delete')"
            href=""
            title="software Delete"
...
export default {
  ...
  methods: {
    edit(data) {
      console.log(data)
      this.$nuxt.$router.push('/softwares/edit/' + data.id)
    },
