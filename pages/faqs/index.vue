...
<template>
  <div>
    ...
    <div v-else>
      ...
      <v-server-table
        :url="serverurl + 'admin/faqs?type=' + selectData"
        :columns="columns"
        :options="options"
      >
...
export default {
  ...
  data() {
    return {
      ...
      error: false,
      selectData: 'all',
      columns: ['id', 'title', 'status', 'actions'],
      options: {
        perPage: 10,
  ...
  methods: {
    edit(data) {
      console.log(data)
      this.$nuxt.$router.push('/faqs/edit/' + data.id)
    },
    ...
    handleDelete(data, i) {
      ...
      if (confirm(msg)) {
        ...
        this.$axios
          .delete(
            '/admin/faqs/' + data.id + '?forcedelete=' + forcedelete.toString()
          )
          .then((res) => {
            vm.loading = false
